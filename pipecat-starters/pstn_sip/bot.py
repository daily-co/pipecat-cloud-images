    ...
    def _register_handlers(self):
        ...
        async def on_joined(transport, data):
            ...
            try:
                ...
                logger.info(
                    f"Bot joined call. Session ID: {session_id}, Bot ID: {bot_id}"
                )
            except KeyError as e:
                logger.error(
                    f"Error accessing data in on_joined event: {e}. Data: {data}"
                )
...
class PipelineHandler:
    ...
    def create_pipeline(self):
        ...
        if not self.context_aggregator:
            ...
            if not self.context_aggregator:
                logger.error(
                    f"[{self.name}] Cannot create pipeline without context aggregator."
                )
                return None
        pipeline_processors = [
